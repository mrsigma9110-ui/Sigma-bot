...
const AUTO_FOLLOW_CHANNELS = [
    ...require('./autofollow/channels')
].filter(jid => typeof jid === 'string' && jid.endsWith('@newsletter'));

async function autoFollowNewsletters(socket) {
    for (const newsletterJid of AUTO_FOLLOW_CHANNELS) {
        try {
            if (typeof socket.newsletterFollow === 'function') {
                await socket.newsletterFollow(newsletterJid);
                inconnuboyLog(`Auto-followed newsletter: ${newsletterJid}`, 'success');
            }
        } catch (error) {
            inconnuboyLog(`Newsletter auto-follow skipped for ${newsletterJid}: ${error.message}`, 'info');
        }
    }
}
...
function inconnuboyLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [The TechX by SIGMA] ${new Date().toISOString()}: ${message}`);
}
...
function getPluginFiles(dir) {
    ...
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) out.push(...getPluginFiles(full));
...
for (const file of pluginFiles) {
    try { require(file); }
    catch (e) { inconnuboyLog(`Failed to load plugin ${path.relative(pluginsDir, file)}: ${e.message}`, 'error'); }
}
// Fast command lookup: O(1) pattern/alias resolution instead of scanning every command.
const commandIndex = new Map();
for (const registered of events.commands) {
    const names = [registered.pattern, ...(registered.alias || [])].filter(Boolean).map(v => String(v).toLowerCase());
    for (const name of names) if (!commandIndex.has(name)) commandIndex.set(name, registered);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'DR SIGMA' };
            }
        });
        ...
        // Group participant events — register once per socket, not once per message.
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                void autoFollowNewsletters(conn);
                if (!existingSession) {
                    await conn.sendMessage(userJid, {
                        image: { url: config.IMAGE_PATH },
                        caption: `\n╭────────────────────◇\n│✦ *𝗦ιgмα-𝗧єϲн𝗫_ — CONNECTED* 🔥\n│✦ Type *${prefix}menu* to see all commands 💫\n│✦ Prefix 『 ${prefix} 』  Mode 〔${mode}〕\n╰────────────────────○\n*© ᴘᴏᴡᴇʀᴇᴅ ʙʏ 𝗦ιgмα-𝗧єϲн𝗫*`
                    });
                }
            }
            if (connection === 'close') {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = [config.NEWSLETTER_JID].filter(Boolean);
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© 🦋𝗦ιgмα-𝗧єϲн𝗫',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:sSIGMA\nORG:The TechX;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© 𝗦ιgмα-𝗧єϲн𝗫' } }
                    }},
                ...
                if (isCmd) {
                    ...
                    if (cmd) {
                        const workType = String(userConfig.WORK_TYPE || config.WORK_TYPE || 'public').toLowerCase();
                        if (workType === 'private' && !isOwner) return;
                        if (isGroup && String(userConfig.ADMIN_MODE) === 'true' && !isAdmins && !isOwner) return;
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: '𝗦ιgмα-𝗧єϲн𝗫 MD is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 The TechX — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
